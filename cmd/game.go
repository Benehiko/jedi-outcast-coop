	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			rows := []config.Row{
				...
				config.NewFloatRow("Blaster spread",
					"Player primary spread half-angle in degrees. 0 = pinpoint; retail sprays 0.5.",
					false, &cfg.Game.BlasterSpread, 0.0, 2.0, 0.1),
				config.NewIntRow("Blaster damage",
					"Player primary damage per bolt. 0 = use the game's own value (retail 20).",
					false, &cfg.Game.BlasterDamage, 0, 200, 5, ""),
				config.NewBoolRow("Blaster knockback",
					"Player primary hits shove living targets. Off = retail (push on kill only).",
					false, &cfg.Game.BlasterKnockback),
